...
for (const { page, lang, param } of pages) {
  ...
  if (lang === 'en') {
    const visible = html
      ...
      /* Титулярят на банковата сметка НЕ се превежда на нито един език.
         Това е регистрирано юридическо име и банката сверява точно него -
         "преведено" име може да върне превода обратно на клиента. */
      .replace(/<div class="bankrow">[\s\S]*?<\/div>/gi, '');
    if (/[Ѐ-ӿ]/.test(visible)) {
