...
for (const { page, lang, param } of pages) {
  ...
  /* 7: кирилица в английска страница. Коментарите и вграденият речник се
     махат: в речника българският стои законно като резервен вариант. */
  if (lang === 'en') {
    const visible = html
      .replace(/<!--[\s\S]*?-->/g, '')
      .replace(/<script>window\.I18N=[\s\S]*?<\/script>/, '');
    if (/[Ѐ-ӿ]/.test(visible)) {
